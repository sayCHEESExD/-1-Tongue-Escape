import { NO_TRAIL, trailBySlot, type TrailStyle } from '@tongue/shared';
import {
  AdditiveBlending,
  BufferAttribute,
  BufferGeometry,
...
export const tintTrail = (
  ...
  if (style === 'rainbow') out.setHSL(((index / count) * 0.8 + time * 0.35) % 1, 0.95, 0.55);
  else out.copy(base);
  return out;
};
...
export class TrailEffect {
  ...
  private tint(index: number, life: number): void {
    ...
    this.scratch.multiplyScalar(life);
  }
}

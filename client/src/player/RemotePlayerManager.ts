import { FULL_EFFECT_REMOTE_PLAYERS, VISIBLE_REMOTE_PLAYERS } from '@tongue/shared';
...
/** The tongue particle budget for a visible remote outside the nearest few. */
 ...
 * nearest run their tongue particles at full budget - so fifteen throwing
...
export class RemotePlayerManager {
  ...
  private rank(local: Vector3): void {
    ...
    for (let i = 0; i < scored.length && winners.size < VISIBLE_REMOTE_PLAYERS; i += 1) {
      ...
      // The nearest few get full tongue particles; the rest a fraction.

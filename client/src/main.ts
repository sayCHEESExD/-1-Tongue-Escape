...
const main = async (): Promise<void> => {
   ...
   */
  setBootStatus('Heating the lava…');
  await paint();
  ...
  setBootStatus('Loading runner…');
  game.loadingStep('Loading runner…');
  await paint();
  ...
  if (clientConfig.debug) {
    ...
    (window as Window & { __tongue?: DebugHandle }).__tongue = { game, loop };
  }

  // Hidden only on a REAL join. Speed, levels, Wins and upgrades are all

...
export type SoundName =
  /** The throw: the tongue leaving the mouth. */
  | 'tongue'
  /** The splat: the tongue slapping onto its target. */
  | 'attach'
  | 'land'
  ...
  | 'rebirth'
  | 'claim'
  /** A tongue or a trail bought. */
  | 'unlock'
  /** Something equipped. */
  | 'buy'
  /** Not enough Wins, or a locked treadmill. */
  | 'refuse';
  ...
  play(name: SoundName, intensity = 1): void {
    ...
    switch (name) {
      case 'tongue':
        ...
        break;
      case 'attach':
        // THE SPLAT: a quick low slap and a falling squelch.
        this.thud(now, 0.4 + level * 0.3, 140);
        this.blip(now, 'sine', 520, 120, 0.16, 0.35 * level);
        break;
      case 'refuse':

...
const PLAYER_ID_KEY = 'tongue.playerId';
...
export class NetworkClient {
  ...
  /** "I am on this tongue's pad." The server checks the position and the price. */
  tonguePad(slot: number): void {
    const message: TonguePadMessage = { slot };
    this.room?.send(MessageType.TonguePad, message);
  }
  ...
  get leaderboard(): LeaderboardSnapshot | null {
    ...
    return { rebirths: copy(board.rebirths), wins: copy(board.wins), tongue: copy(board.tongue) };
  }

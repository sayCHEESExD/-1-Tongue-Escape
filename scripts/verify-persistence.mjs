...
/** Earn some XP the only way there is: by WALKING. Returns the new total. */
const earn = async (room, seconds = 1.2) => {
  const before = me(room).xp;
  let seq = room.nextSeq ?? 1;
  const steps = Math.round(seconds / 0.05);
  for (let i = 0; i < steps; i += 1) {
    room.send('move', { seq: seq++, dt: 0.05, moveX: 0, moveZ: 1, tongue: false, cameraYaw: 0 });
    await sleep(50);
  }
  room.nextSeq = seq;
  await waitFor(() => me(room).xp > before, 'XP to be credited', 8000);
  // Back to spawn, so the next stretch of running does not walk off the hub.
  room.send('requestRespawn', {});
  await sleep(150);
  return me(room).xp;
};
...
const PROGRESS_KEYS = [
  'xp', 'lifetimeXp', 'wins', 'ownedTongues', 'tongueSlot', 'rebirths',
  'ownedTrails', 'trailSlot', 'bestStage', 'playSeconds',
];
...
class Mongod {
  constructor(binary, dbPath, port) {
    ...
    this.dbPath = dbPath;
    this.port = port;
    this.uri = `mongodb://127.0.0.1:${port}/tongue_verify`;
  }
  async start() {
...
const runSuite = async (ctx) => {
  ...
  console.log(`\n${kind}: guests`);
  {
    ...
    const stored = await waitForProfile(reader, 'p_guest1', (p) => p.xp >= t1, 'guest save');
    check(stored.displayName === 'GuestOne', 'the guest save keeps its display name');
    const again = await joinAs(port(), { playerId: 'p_guest1' });
    check(me(again).xp === stored.xp, `existing guest profile restores (xp ${stored.xp})`);
  ...
  console.log(`\n${kind}: first login migrates`);
  {
    ...
    check(me(s).xp === before.xp, 'the session carries the guest progress into the account');
    ...
    check(guest.migratedSnapshot?.xp === before.xp, 'the guest copy keeps its progress as migratedSnapshot');
    check(guest.xp === 0, 'the retired guest profile is fresh');
    const more = await earn(s);
    check(more > before.xp, 'the signed-in session keeps earning');
    await leaveAndWait(s);
    const after = await waitForProfile(reader, acctKey, (p) => p.xp >= more, 'account save after playing');
    check((await profileOf(reader, 'p_guest1')).xp === 0, 'account play does not touch the retired guest');
    ctx.accountTotal = after.xp;
  ...
  console.log(`\n${kind}: nothing leaks to a claimed id`);
  {
    ...
    check(me(raw).xp === 0, 'a raw account id as a browser id gets a fresh guest, nothing more');
    await leaveAndWait(raw);
    const forged = await joinAs(port(), { playerId: 'p_forger', token: `forged:${acct}` });
    check(me(forged).xp === 0, 'a forged token gets a fresh guest, nothing more');
    await leaveAndWait(forged);
    const noid = await joinAs(port(), { playerId: 'p_noid', token: `noid:${acct}` });
    check(me(noid).xp === 0 && server.current.log.includes('carried no usable _id'), 'a 2xx without an _id does not verify');
  ...
  {
    const s = await joinAs(port(), { playerId: 'p_guest1', token: `valid:${acct}` });
    check(me(s).xp === ctx.accountTotal, 'reconnecting as the same account restores it');
    const out = await setAuth(s, null, 'guest');
    check(out.status === 'guest' && (await settled(s, (p) => p.xp === 0, 'fresh guest state')), 'logout -> guest, fresh because this browser was migrated');
    const guestBit = await earn(s, 0.8);
    const back = await setAuth(s, `valid:${acct}`, 'account');
    check(back.status === 'account' && (await settled(s, (p) => p.xp === ctx.accountTotal, 'account state')), 'signing back in restores the account');
    // The switch saved the guest it LEFT from live state before loading the account.
    const guest = await waitForProfile(reader, 'p_guest1', (p) => p.xp >= guestBit, 'the leaving guest save');
    check(guest.migratedTo === acctKey && guest.xp >= guestBit, 'the retired guest is not migrated a second time');
    check((await profileOf(reader, acctKey)).xp === ctx.accountTotal, 'guest play never reached the account');
    await leaveAndWait(s);
  }
  ...
  console.log(`\n${kind}: the same account elsewhere`);
  {
    ...
    check(me(other).xp === ctx.accountTotal, 'the same account on another browser sees the same progress');
  ...
  console.log(`\n${kind}: an existing account is never overwritten`);
  {
    ...
    await waitForProfile(reader, 'p_guest3', (p) => p.xp >= t3, 'guest3 save');
    const s = await joinAs(port(), { playerId: 'p_guest3', token: `valid:${acct}` });
    check(me(s).xp === ctx.accountTotal, 'the account wins over the browser profile');
    const guest3 = await profileOf(reader, 'p_guest3');
    check(guest3.xp >= t3 && !guest3.migratedTo, 'the browser profile is left untouched and not retired');
    const out = await setAuth(s, null, 'guest');
    check(out.status === 'guest' && (await settled(s, (p) => p.xp === guest3.xp, 'guest3 state')), "logout returns that browser's own progress");
    await leaveAndWait(s);
  }

  console.log(`\n${kind}: purchases`);
  {
    const first = await webhook(port(), { transactionId: 'tx-1', userId: acct, sku: 'wins_small', username: 'user_acct1' });
    check(first.status === 200 && first.body?.ok === true, 'the webhook answers 200 once the grant is recorded');
    const dup = await webhook(port(), { transactionId: 'tx-1', userId: acct, sku: 'wins_small' });
    ...
    await sleep(400);
    check(me(impostor).wins === 0, 'a guest using the account id as a browser id is paid nothing');
    await leaveAndWait(impostor);
    const winsBefore = (await profileOf(reader, acctKey)).wins;
    const s = await joinAs(port(), { playerId: 'p_guest1', token: `valid:${acct}` });
    await waitFor(() => me(s).wins === winsBefore + 250, 'the grant to be paid on join', 8000);
    pass('a recorded purchase is paid to the verified account on join (+250)');
    await waitForProfile(reader, acctKey, (p) => p.wins === winsBefore + 250, 'account save with the grant');
    ...
    const second = await webhook(port(), { transactionId: 'tx-2', userId: acct, sku: 'wins_large' });
    ...
    await waitFor(() => me(again).wins === winsBefore + 250 + 1500, 'the grant to survive the restart', 8000);
    pass('a purchase recorded before a restart is paid after it (+1500)');
    await waitForProfile(reader, acctKey, (p) => p.wins === winsBefore + 250 + 1500, 'account save after the restart grant');
    await leaveAndWait(again);
    ctx.accountWins = winsBefore + 250 + 1500;
  }

  console.log(`\n${kind}: Bloxity unavailable`);
  {
    const s = await joinAs(port(), { playerId: 'p_outage', token: 'down:acct9' });
    check(me(s).xp === 0 && server.current.log.includes('bloxity unavailable, will re-ask'), 'a token Bloxity cannot answer for is let in as a guest and re-asked later');
    await leaveAndWait(s);
    const h = await joinAs(port(), { playerId: 'p_outage2', token: 'neterr:acct9' });
    check(me(h).xp === 0, 'a network error verifying is a guest too');
  ...
  console.log(`\n${kind}: a server restart loses nothing`);
  {
    ...
    await leaveAndWait(g);
    await waitForProfile(reader, 'p_restart', (p) => p.xp >= t, 'save before restart');
    const accountBefore = await profileOf(reader, acctKey);
    await restart();
    const again = await joinAs(port(), { playerId: 'p_restart' });
    check(me(again).xp >= t, 'a guest profile survives a server restart');
    await leaveAndWait(again);
    const s = await joinAs(port(), { playerId: 'p_browser2', token: `valid:${acct}` });
    check(me(s).xp === accountBefore.xp && me(s).wins === accountBefore.wins, 'an account profile survives a server restart');
    await leaveAndWait(s);
  }
  ...
  console.log(`\n${kind}: the wrong game`);
  {
    ...
    check(me(s).xp === 0 && server.current.log.includes('rejected (401 GAME_TOKEN_INVALID)'), 'a token for another game slug is rejected by the verify route');
    await leaveAndWait(s);
    await server.current.stop();
    server.current = await ctx.startServer();
  }
...
const runJsonExtras = async (ctx, server) => {
  ...
  console.log('\njson: a corrupt file is moved aside');
  {
    ...
    await earn(g, 0.6);
    await leaveAndWait(g);
    await waitForProfile(reader, 'p_after_corrupt', (p) => p.xp > 0, 'a save after the move-aside');
...
const runMongoExtras = async (ctx, server, mongod) => {
  ...
  console.log('\nmongo: the database goes down');
  {
    ...
    const stayed = stay.status === 'account' && /staying/.test(stay.note ?? '') && me(s).xp === total;
    check(stayed, 'a sign-out that cannot reach storage stays on the account');
    if (!stayed) {
      note(`authState=${JSON.stringify(stay)} total=${total} now=${me(s).xp}`);
    ...
    const landed = await waitForProfile(reader, acctKey, (p) => p.xp >= more, 'the queued save to land once the database is back', 40_000);
    check(landed.xp >= more, 'a save made during the outage lands once the database is back');
    const again = await joinAs(port(server), { playerId: 'p_browser2', token: `valid:${acct}` });
    check(me(again).xp === landed.xp && me(again).wins === ctx.accountWins, 'everything is intact after the outage');
    const out = await setAuth(again, null, 'guest');
    const wentThrough = out.status === 'guest' && (await settled(again, (p) => p.xp === 0, 'guest after outage'));
    check(wentThrough, 'and the sign-out goes through now');
    if (!wentThrough) {
      note(`authState=${JSON.stringify(out)} now=${me(again).xp} guestDoc=${JSON.stringify(await profileOf(reader, 'p_browser2'))}`);
      for (const line of server.current.lines.slice(-12)) note(line);
    }
    await leaveAndWait(again);
  }
  ...
  console.log('\nmongo: the legacy JSON file is imported, insert-only');
  {
    ...
    const legacy = {
      legacy_player: { ...ctx.guestProgress, xp: 777, wins: 7, displayName: 'Legacy', updatedAt: 1 },
      [acctKey]: { ...account, xp: 1, wins: 1, displayName: 'MUST NOT WIN', updatedAt: Date.now() + 1e9 },
    };
    ...
    check(imported?.xp === 777 && imported?.wins === 7, 'a profile only the legacy file knew is added');
    const kept = await profileOf(reader, acctKey);
    check(kept.xp === account.xp && kept.displayName !== 'MUST NOT WIN', 'an existing account profile is never overwritten by the import');
...
const run = async () => {
  ...
  if (process.env.ONLY !== 'mongo') {
    const dataDir = mkdtempSync(path.join(tmpdir(), `tongue-json-${stamp}-`));
  ...
  if (!uri) {
    ...
    if (binary) {
      mongod = new Mongod(binary, mkdtempSync(path.join(tmpdir(), `tongue-mongod-${stamp}-`)), MONGO_PORT);
  ...
  if (!uri) {
    ...
    const dataDir = mkdtempSync(path.join(tmpdir(), `tongue-mongo-${stamp}-`));

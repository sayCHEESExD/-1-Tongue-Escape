 ...
 * and tongue state. Because the very same `stepPlayer` runs on the client
...
export class MovementService {
  ...
  /** True when the last step began AND ended on the ground, walking: a real stride. */
  lastStepOnGround(player: PlayerState): boolean {
    return this.lastWasGrounded && player.grounded && player.tonguePhase === 0;
  }
  ...
  private publish(player: PlayerState, sim: Sim): void {
    ...
    player.tonguePhase = m.tonguePhase;

import { LEADERBOARD_SIZE, handleFor, resolveLevel, tongueLengthFor } from '@tongue/shared';
...
/**
 * The three boards on the back wall: Top Rebirths, Top Wins and Top Tongue
 * (the current Tongue Length).
 *
  ...
  rebuild(
    ...
    for (const [id, profile] of profileStore.entries()) {
      ...
      byHandle.set(handle, {
        ...
        tongue: tongueLengthFor(resolveLevel(profile.xp ?? 0).level),
      });
    }
    ...
    fill(board.tongue, all, (c) => c.tongue);
  }
}

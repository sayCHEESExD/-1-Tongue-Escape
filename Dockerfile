...
# monorepo and the server imports `@tongue/shared` as a workspace dependency;
...
#   docker run -e PORT=2586 -p 2586:2586 tongue-server
...
# imports require: `@tongue/shared` resolves to `shared/dist/index.js`.
RUN npm run build:server

# Drop to production dependencies in place. This keeps the workspace symlinks
# that `@tongue/shared` resolves through - deleting node_modules and
...
# hosts, injects one - and `serverConfig` falls back to 2586.
ENV HOST=0.0.0.0
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:'+(process.env.PORT||2586)+'/health').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"
